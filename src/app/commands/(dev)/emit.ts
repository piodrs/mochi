...
export const chatInput: ChatInputCommand = async ({ interaction, client }) => {
    ...
    if (!guild) {
        await interaction.reply(res.error("Este comando só pode ser usado em um servidor.", true));
        return;
    }
    ...
    switch (options.getSubcommand(true)) {
        case "join":
            ...
            await interaction.reply(res.success(`${emojis.success} Simulei o evento **join** para ${member}.`, true));
            break;
    }
};