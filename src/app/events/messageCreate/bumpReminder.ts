...
function getBumpSource(authorId: string): BumpSource | null {
    const entries = Object.entries(guild.bumps) as [BumpSource, (typeof guild.bumps)[BumpSource]][];

    for (const [source, config] of entries) if (config.botId && config.botId === authorId) return source;

    return null;
}

function isDisboardBumpMessage(message: Parameters<EventHandler<"messageCreate">>[0]): boolean {
...
function isDiscadiaBumpMessage(message: Parameters<EventHandler<"messageCreate">>[0]): boolean {
    return message.content.toLowerCase().includes("has been successfully bumped");
}

function isValidBumpConfirmation(source: BumpSource, message: Parameters<EventHandler<"messageCreate">>[0]): boolean {
    switch (source) {
        case "disboard": return isDisboardBumpMessage(message);
        case "discadia": return isDiscadiaBumpMessage(message);
    }
}
...
const handler: EventHandler<"messageCreate"> = async (message) => {
    ...
    const nextBumpAt = new Date(now.getTime() + (config.cooldownHours * 60 * 60 * 1000));
