...
function createGuildRecord(id: string, document: Awaited<ReturnType<typeof db.guilds.get>>) {
  return {
    id,
    document,
    data: document?.data,
...
function isReminderDue(state: GuildBumpState, now: Date) {
  ...
  if (state.lastReminderSentAt && state.lastReminderSentAt.getTime() >= state.nextBumpAt.getTime()) return false;
...
export async function saveGuildBumpState(guild: Guild, source: BumpSource, now: Date, nextBumpAt: Date) {
...
export async function markGuildBumpReminderSentById(guildId: string, source: BumpSource, sentAt = new Date()) {
...
export async function getDueGuildBumpReminders(now: Date) {
  ...
  for (const document of documents) {
    ...
    const entries = Object.entries(bumpState) as [BumpSource, GuildBumpState | undefined][];
