...
function createMemberRecord(id: string, document: Awaited<ReturnType<typeof db.members.get>>) {
  return {
    id,
    document,
    data: document?.data,
...
export async function registerMemberBump(member: GuildMember, source: BumpSource, bumpedAt: Date) {
